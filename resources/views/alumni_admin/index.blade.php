...
<div class="container my-5">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2 class="fw-bold text-dark">
            <i class="bi bi-trophy-fill me-2 text-warning"></i> Daftar Alumni
        </h2>
        <a href="{{ route('alumni.create') }}" class="btn btn-primary shadow-sm">
            ...
            <tbody>
                @forelse ($datas as $data)
                    <tr>
                        ...
                        <td>
                            <img src="{{ asset('storage/' . $data->fotoalum) }}"
                                 alt="{{ $data->namaalum }}"
                                 class="rounded"
                                 style="width: 60px; height: 60px; object-fit: cover;">
                        </td>
                        <td class="fw-bold">{{ $data->namaalum ?? "category tidak ada" }}</td>
                        <td class="fw-bold">{{ $data->category->category ?? "category tidak ada" }}</td>
                        <td style="max-width: 300px;" class="text-truncate" title="{{ $data->deskripsialum }}">
                            {{ $data->deskripsialum }}
                        </td>
                        <td class="fw-bold">{{ $data->rating }} ⭐</td>
                        <td class="fw-bold">{{ $data->angkatan }}</td>
                        <td>
                            <a href="{{ route('alumni.edit', $data->id) }}"
                               class="btn btn-sm btn-warning">
                                <i class="bi bi-pencil-square"></i> Edit
                            </a>
                            <a href="{{ route('alumni.show', $data->id) }}"
                               class="btn btn-sm btn-secondary">
                                <i class="bi bi-pencil-square"></i> Show
                            </a>
                            <form action="{{ route('alumni.destroy', $data->id) }}" 
